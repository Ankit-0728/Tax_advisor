 ...
 var runSlider = function(container,opt) {
	...
	if (!container.hasClass("revslider-initialised")) {
		...
		if (versionTop==1 && versionMinor < 7) 
			container.html('<div style=" padding:40px 0px; font-size:20px; color:#992222;"> The Current Version of jQuery:'+version+' <br>Please update your jQuery Version to min. 1.7 in Case you wish to use the Revolution Slider Plugin</div>');									
	...
	nextli.find('img,.defaultimg, .tp-svg-layer').each(function() {
		...
		if (opt.youtubeapineeded == true && (!window['YT'] || YT.Player==undefined)) {		
			...
			if (jQuery.now()-opt.youtubestarttime>5000 && opt.youtubewarning!=true) {
				...
				console.error(txt); 
				opt.c.append('<div style="position:absolute;top:50%;width:100%;color:#e74c3c;  font-size:16px;  padding:15px;background:#000; display:block;"><strong>'+txt+'</strong></div>')				 				
			}
		}
